...
### Prometheus
- Scrapes metrics from job platform nodes every 15 seconds
- All nodes (API, worker, or combined) expose metrics on port 8080
- Profiles are stackable: use `api` + `worker` together for nodes that do both
